        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mt-12 space-y-4">
            {[
              ...
              <a key={doc.title} href={doc.href} className="flex items-start gap-4 bg-gray-50 rounded-2xl border border-gray-100 p-6 hover:shadow-lg transition-shadow group">
                <div className="w-12 h-12 rounded-xl bg-indigo-50 text-[var(--primary)] flex items-center justify-center flex-shrink-0">
                  {doc.icon}
                </div>
                <div className="flex-1">
                  <h2 className="text-lg font-semibold text-gray-900 group-hover:text-[var(--primary)] transition-colors">{doc.title}</h2>
                  <p className="mt-1 text-sm text-gray-500">{doc.description}</p>
                </div>
                <svg className="w-5 h-5 text-gray-400 group-hover:text-[var(--primary)] transition-colors flex-shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              </a>
            ))}
          </div>
