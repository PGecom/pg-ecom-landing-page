...
export default function PrepaidHero() {
  const { t } = useTranslation();

  return (
    <section className="relative pt-32 pb-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-20 left-1/4 w-72 h-72 bg-orange-200 rounded-full opacity-20 blur-3xl" />
        <div className="absolute top-40 right-1/4 w-96 h-96 bg-amber-200 rounded-full opacity-20 blur-3xl" />
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-full px-4 py-1.5 mb-6">
            ...
            <span className="text-sm font-medium text-amber-700">{t.prepaid.hero.badge}</span>
          </div>
          ...
          <h1 className="font-[family-name:var(--font-montserrat)] text-4xl sm:text-5xl lg:text-[3.5rem] font-extrabold text-gray-900 leading-[1.1]">
            {t.prepaid.hero.headingLine1}{" "}
            <span className="gradient-text">{t.prepaid.hero.headingHighlight}</span>
          </h1>
        ...
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto">
          {t.prepaid.metrics.map((metric, index) => (
