...
export default function PrepaidHowItWorks() {
    ...
    <section className="py-24 px-4 sm:px-6 lg:px-8 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="text-sm font-semibold text-[var(--primary)] uppercase tracking-wide">{t.prepaid.howItWorks.sectionLabel}</span>
          <h2 className="font-[family-name:var(--font-montserrat)] mt-3 text-3xl sm:text-4xl font-bold text-gray-900">{t.prepaid.howItWorks.heading}</h2>
        </div>

        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {t.prepaid.howItWorks.steps.map((step, index) => (
            <div key={index} className="relative text-center">
              <div className="w-16 h-16 rounded-2xl bg-indigo-50 text-[var(--primary)] flex items-center justify-center mx-auto mb-6">
                {stepIcons[index]}
              </div>
              <div className="absolute top-8 left-1/2 w-full h-px bg-gray-200 -z-10 hidden md:block" style={{ display: index === 2 ? "none" : undefined }} />
              <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-[var(--primary)] text-white text-sm font-bold mb-4">
                {index + 1}
              </span>
